 ...
 * @see Tragamonedas
 */
public class TragamonedasProgresiva extends Tragamonedas {
     ...
     */
    public TragamonedasProgresiva(String nombre) {
        super(nombre, 5, 500, 50000);
        this.boteProgresivo = 50000;
        this.incrementoPorcentaje = 0.05;
        inicializarSimbolos();
        inicializarTambores();
    }
    ...
    public double girar(double apuesta) {
        ...
        boolean todosJackpot = resultados.stream().allMatch(s -> s.getNombre().equals("Jackpot"));
        if (todosJackpot) {
    ...
    public double getMultiplicadorEspecial(List<Simbolo> simbolos) {
        // Combinacion de Jackpot (manejo especial en girar)
        if (simbolos.stream().allMatch(s -> s.getNombre().equals("Jackpot"))) {
            return 0; // Se maneja en girar
        }
